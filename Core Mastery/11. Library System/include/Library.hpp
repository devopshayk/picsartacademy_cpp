...
class Library {
    ...
    public:
        ...
        void addBook(size_t shelfIndex, int id, const std::string& title, const std::string& authorName);

        Book* findBookByTitle(const std::string& title);
        void removeBookByTitle(const std::string& title);

        void transferBook(const std::string& title, size_t from, size_t to);

        void replaceBook(int id, const std::string& newTitle, const std::string& newAuthor);
